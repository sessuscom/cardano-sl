...
\begin{equation*}
\forall (\mathit{tx}, i)  \mapsto (\mathit{addr}, c) \in u \cdot
...
\begin{equation*}
\forall (\mathit{tx}, i)  \mapsto (\mathit{addr}, c) \in u \cdot
\forall (\mathit{tx}, i') \mapsto (\mathit{addr}', c') \in v \cdot
...
\begin{equation*}
\exists \mathit{ix} \cdot (\mathsf{txid} ~ t_1, \mathit{ix}) \in \mathsf{txins} ~ t_2
\end{equation*}
\end{definition}
...
this specification. Note that this covers the case of one of our own
...
can only spent outputs in the wallet's current UTxO. Alternatively, we could
require that
%
\begin{equation*}
...
\begin{invariant}[Transactions removed from pending once included in UTxO]
\begin{equation*}
...
The basic model is intended to be basic, not efficient. Let us take the
...
that are not available in wallet's UTxO (we will explore this issue in detail in
...
However, rollbacks also mean that we have end up with pending transactions that
...
in the blockchain, but haven't yet. We will refer to the curresponding unspent
...
subsets of confirmed pending and expected transactions as ``possible futures''.
...
economical concept of ``value at risk''.}
...
For somewhat similar reasons, the wallet's \emph{maximum} balance
...
obviously compatible with each other. However, keep track of this information
does not help us here: we already assume that \emph{all} expected transactions
...
History tracking refers to the wallet keeping some additional information about
transactions and blocks. From the point of the abstract wallet specification
we are primarily interested in modelling how this relates to rollback. The
basic model is shown in Figure~\ref{fig:history_tracking}. We have two parameters:
...
& \qquad (\mathit{utxo}, pending \cup \{ tx \} ) : \mathit{checkpoints}, \mathit{txInfo} \cup \{ \mathsf{txid} ~ \mathit{tx} \mapsto \mathsf{txMeta} ~ \mathit{tx} \}) \\
...
\end{align*}
%
\caption{\label{fig:history_tracking}History tracking}
\end{figure}

\subsection{Relation to the current V1 Wallet API}

The model proposed in this section is sufficiently expressive to encode what
the current V1 wallet REST API requires.

\begin{itemize}
\item Most information returned is entirely static, and can be covered by
a suitable choice for $\mathsf{TxMeta}$. This includes the transaction ID,
the total amount, the inputs and outputs (both in terms of addresses),
whether the transaction is ``local'' or not (meaning it is from and to
addresses owned by the wallet), the transaction direction (incoming or
outgoing), and the transaction creation time.
\item The depth of the transaction in the blockchain can be recorded
(somewhat implicitly) by choosing
\begin{equation*}
\mathsf{BlockMeta} = \mathsf{TxId} \mapsto \mathsf{BlockNumber}
\end{equation*}
The depth can then be computed provided the wallet knows the length of the
current chain.
\item The transaction's status can be reported as follows:
%
\begin{equation*}
\begin{cases}
...
\end{cases}
\end{equation*}
The \texttt{Creating} status can be seen as a phase before the transaction
enters the wallet, and is mostly inpdendent from it.
\end{itemize}
...
transactions, the right shows incoming transactions. The empty circle means that
the transaction is not included in the wallet's reported history.
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,
  ...
  \path[every node/.style={font=\sffamily\scriptsize,
    ...
    (LStart)
      edge node {\texttt{newPending}} (LApplying)
    (LApplying)
      edge [bend right=30] node[left=1mm] {\texttt{applyBlock}} (LInNewestBlock)
      edge [bend left=30]  node[right=1mm] {\texttt{applyBlock}$^*$} (LWontApply)
    (LInNewestBlock)
      edge [loop left]     node {\texttt{applyBlock}} (LInNewestBlock)
      edge [bend right=30] node[left=1mm] {\texttt{rollback}} (LApplying)
      edge node {\texttt{applyBlock}} (LPersisted)
    (LWontApply)
      edge [bend left=30] node[right=1mm] {\texttt{rollback}$^*$} (LApplying)
    ;
  ...
  \path[every node/.style={font=\sffamily\scriptsize,
    ...
    % achieve proper rendering of labels over arrows.
    (RStart)
      edge [bend right=30] node[left=1mm] {\texttt{applyBlock}} (RInNewestBlock)
    (RInNewestBlock)
      edge [loop right]     node {\texttt{applyBlock}} (RInNewestBlock)
      edge [bend right=30] node[right=1mm] {\texttt{rollback}} (RStart)
      edge node {\texttt{applyBlock}} (RPersisted)
    ;
\end{tikzpicture}
\caption{\label{fig:transaction_state}Transaction state transitions}
\end{figure}
...
\subsection{Transaction resubmission}

In an actual wallet implementation, $\mathsf{newPending}$ would of course have to
broadcast the transaction over the network, so that a next slot leader can
include the transaction into the blockchain. The wallet may wish to monitor this,
resubmit the transaction again if it hasn't been included after a while, and
eventually give up.
